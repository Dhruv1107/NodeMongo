...
class Product {
  constructor(title, price, description, imageUrl, id) {
    ...
    // If id exists we create OnjectId 
  }

  save() {
    const db = getDb();
    let dbOp;
    if(this._id) { //If id is set update the product
      dbOp = db
      .collection('products')
    ...
    else { // If id is not set delete the product
      dbOp = db
      .collection('products') //If the collection doesn't exist it creates the connection
